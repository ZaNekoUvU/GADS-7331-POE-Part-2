# Documentation index

This folder is the **documentation section** for **GADS 7331 POE Part 2 — Back To The Forge**.

| File | Format | Contents |
|------|--------|----------|
| [context.md](context.md) | Markdown | Project context: overview, architecture, playtest state, priorities |
| [high-concept.txt](high-concept.txt) | Plain text | Ideation, LLM role, rationale for local models |
| [setup.md](setup.md) | Markdown | Unity + Ollama setup, specs, troubleshooting |
| [refinements-changes.md](refinements-changes.md) | Markdown | Log of refinements and AI-assisted decisions |
| [ollama-plan.md](ollama-plan.md) | Markdown | Model choice, inference timing, data flow, prompts, risks |
| [prompts-used.md](prompts-used.md) | Markdown | Prompt archive: verbatim prompts, success/fail examples, iterations |

Project overview and install summary: [../README.md](../README.md).
